...
#-----------------------------------------------1-1.左カメラ（基準カメラ）
for fn in glob("./data/cal_0/calibration0/*.jpg"):
    # 画像の取得
    im = cv2.imread(fn, 0)
    print("loading..." + fn)
    # チェスボードのコーナーを検出
    ...
    if found:
        ...
        cv2.cornerSubPix(im, corner, (11,11), (-1,-1), term)    #よくわからないがサブピクセル処理（小数点以下のピクセル単位まで精度を求める）
...
for i in range(N):
    # 画像の取得
    im_l = cv2.imread("./data/cal_0/calibration0/cal_cam0_"+str(i*3)+ ".jpg", 0)
...
distCoeffs1 = d_l
distCoeffs2 = d_r
im_l = cv2.imread("./data/cal_0/calibration0/cal_cam0_0.jpg", 0)
h, w = im_l.shape[:2]
